function FcmResult(varnamesel, stats, class, names, bins, fout, fmodel, varargin)
	...
	while i <= size(varargin, 2)
        ...
        if ~argok,
            disp (['(fcmresult) ignoring invalid argument: #', num2str(i+1)]);
        end
        i = i + 1;
	end
	...
	szStr = [szStr, ' Correct rate: ', num2str(stats.correct_rate), ...
			' and variable number: ', num2str(stats.varnum), ' Class Map: ', num2str(stats.CMap) ];
