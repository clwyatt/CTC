...
for rr=1:length(worklist)
    ...
    add_act_tr = [bioact_tr];
    add_bin_tr = [bin_tr];
    add_results_bin_tr = [results_bin_tr];
    add_results_tr = [results_tr(:,Inc)]; 
    ...
    for xx = 1:Rep
        ...
        if xx==1
            for Hid=2:12
                net = newff([InputMin InputMax],[length(Inc) Hid 1],{'tansig' 'tansig' 'purelin'}, 'trainlm','learngdm');
                ...
                acc_net_ensemble_tr(Hid-1) = sum(net_ensemble_bin_tr(:,xx) == bin_tr)/length(bin_tr);
            end;
            [sorted,Ind] = sort(acc_net_ensemble_tr);
            HiddenLayer = Ind(end)+1
        end;         
        net = newff([InputMin InputMax],[length(Inc) HiddenLayer 1],{'tansig' 'tansig' 'purelin'}, 'trainlm','learngdm');
        net.trainParam.epochs = 80;
        net.trainParam.show = nan;
        net = train(net,add_results_tr',add_act_tr');
        net_ensemble_tr(:,xx) = sim(net,results_tr(:,Inc)')';
        ...
        rbf = newrb(add_results_tr',add_act_tr',0,4,40,40);
        ...
        if xx==1
            for Hid=2:12
                net2 = newff([InputMin InputMax],[length(Inc2) Hid 1],{'tansig' 'tansig' 'purelin'}, 'trainlm','learngdm');
                ...
                acc_net2_ensemble_tr(Hid-1) = sum(net2_ensemble_bin_tr(:,xx) == bin_tr)/length(bin_tr);
            end;
            [sorted,Ind] = sort(acc_net2_ensemble_tr);
            HiddenLayer2 = Ind(end)+1
        end;    
        net2 = newff([InputMin InputMax],[length(Inc2) HiddenLayer2 1],{'tansig' 'tansig' 'purelin'}, 'trainlm','learngdm');
        net2.trainParam.epochs = 80;
        net2.trainParam.show = nan;
        net2 = train(net2,add_results_bin_tr',add_bin_tr');
        tempres = round(sim(net2,results_bin_tr')');
