...
for rr=1:length(worklist)
    clear avg_*  step_* regr* net* rbf* vote_* *ensemble*
    ...
    results_bin_tr = results_bin_tr(:,Inc);
    results_bin_tst = results_bin_tst(:,Inc);
    ...
    for xx = 1:Rep
        ...
        InputMax = binnum*ones(length(Inc), 1);
        ...
        net2_ensemble_bin_tr(:,xx) = round(sim(net2,results_bin_tr(:,Inc)')');
        tempres = round(sim(net2,results_bin_tst(:,Inc)')');
        ...
        confusion_avg_ensemble_tr = zeros(binnum);
        confusion_avg_ensemble_tst = zeros(binnum);
        confusion_step_ensemble_tr = zeros(binnum);
        confusion_step_ensemble_tst = zeros(binnum);
        confusion_regress_ensemble_tr = zeros(binnum);
        confusion_regress_ensemble_tst = zeros(binnum);
        confusion_net_ensemble_tr = zeros(binnum);
        confusion_net_ensemble_tst = zeros(binnum);
        confusion_rbf_ensemble_tr = zeros(binnum);
        confusion_rbf_ensemble_tst = zeros(binnum);
        confusion_vote_ensemble_tr = zeros(binnum);
        confusion_vote_ensemble_tst = zeros(binnum);
        confusion_net2_ensemble_tr = zeros(binnum);
        confusion_net2_ensemble_tst = zeros(binnum);
        
        for ii=1:binnum 
            for jj=1:binnum
                confusion_avg_ensemble_tr(ii,jj) = confusion_avg_ensemble_tr(ii,jj) + 1/Rep*sum((avg_ensemble_bin_tr(:,xx) == jj).*(bin_tr == ii));
                confusion_avg_ensemble_tst(ii,jj) = confusion_avg_ensemble_tst(ii,jj) + 1/Rep*sum((avg_ensemble_bin_tst(:,xx) == jj).*(bin_tst == ii));
                confusion_step_ensemble_tr(ii,jj) = confusion_step_ensemble_tr(ii,jj) + 1/Rep*sum((step_ensemble_bin_tr(:,xx) == jj).*(bin_tr == ii));
                confusion_step_ensemble_tst(ii,jj) = confusion_step_ensemble_tst(ii,jj) + 1/Rep*sum((step_ensemble_bin_tst(:,xx) == jj).*(bin_tst == ii));
                confusion_regress_ensemble_tr(ii,jj) = confusion_regress_ensemble_tr(ii,jj) + 1/Rep*sum((regress_ensemble_bin_tr(:,xx) == jj).*(bin_tr == ii));
                confusion_regress_ensemble_tst(ii,jj) = confusion_regress_ensemble_tst(ii,jj) + 1/Rep*sum((regress_ensemble_bin_tst(:,xx) == jj).*(bin_tst == ii));
                confusion_net_ensemble_tr(ii,jj) = confusion_net_ensemble_tr(ii,jj) + 1/Rep*sum((net_ensemble_bin_tr(:,xx) == jj).*(bin_tr == ii));
                confusion_net_ensemble_tst(ii,jj) = confusion_net_ensemble_tst(ii,jj) + 1/Rep*sum((net_ensemble_bin_tst(:,xx) == jj).*(bin_tst == ii));
                confusion_rbf_ensemble_tr(ii,jj) = confusion_rbf_ensemble_tr(ii,jj) + 1/Rep*sum((rbf_ensemble_bin_tr(:,xx) == jj).*(bin_tr == ii));
                confusion_rbf_ensemble_tst(ii,jj) = confusion_rbf_ensemble_tst(ii,jj) + 1/Rep*sum((rbf_ensemble_bin_tst(:,xx) == jj).*(bin_tst == ii));
                confusion_vote_ensemble_tr(ii,jj) = confusion_vote_ensemble_tr(ii,jj) + 1/Rep*sum((vote_ensemble_bin_tr(:,xx) == jj).*(bin_tr == ii));
                confusion_vote_ensemble_tst(ii,jj) = confusion_vote_ensemble_tst(ii,jj) + 1/Rep*sum((vote_ensemble_bin_tst(:,xx) == jj).*(bin_tst == ii));
                confusion_net2_ensemble_tr(ii,jj) = confusion_net2_ensemble_tr(ii,jj) + 1/Rep*sum((net2_ensemble_bin_tr(:,xx) == jj).*(bin_tr == ii));
                confusion_net2_ensemble_tst(ii,jj) = confusion_net2_ensemble_tst(ii,jj) + 1/Rep*sum((net2_ensemble_bin_tst(:,xx) == jj).*(bin_tst == ii));
            end;
        end;    
    end;
    ...
    F = [err_step_tr stderr_step_tr err_step_tst stderr_step_tst class_step_tr class_step_tst; ...
        ...
        xlswrite(outputfile,F,worksheet,'B2:G13');
    ...
    if ~strcmp(worksheet,'topliss')
        ...
        xlswrite(outputfile,{'Step-Tr'; 'A1'; 'A2'; 'A3'; '';...
            ...
            'A1'; 'A2'; 'A3'; ''; 'FFCNetEns-Tr';'A1'; 'A2'; 'A3'},worksheet,'A23:A81');
        ...
        xlswrite(outputfile,Mconfusion_avg_ensemble_tr,worksheet,'B44:D46');
        xlswrite(outputfile,Mconfusion_regress_ensemble_tr,worksheet,'B49:D51');
        xlswrite(outputfile,Mconfusion_step_ensemble_tr,worksheet,'B54:D56');
        xlswrite(outputfile,Mconfusion_net_ensemble_tr,worksheet,'B59:D61');
        xlswrite(outputfile,Mconfusion_vote_ensemble_tr,worksheet,'B64:D66');
        xlswrite(outputfile,Mconfusion_rbf_ensemble_tr,worksheet,'B69:D71');
        xlswrite(outputfile,Mconfusion_net2_ensemble_tr,worksheet,'B74:D76');
        xlswrite(outputfile,Mconfusion_net1_ensemble_tr,worksheet,'B79:D81');
        ...
        xlswrite(outputfile,{'Step-Tst'; 'A1'; 'A2'; 'A3'; '';...
            ...
            'A1'; 'A2'; 'A3';''; 'FFCNetEns-Tst';'A1'; 'A2'; 'A3'},worksheet,'F23:F81');
        ...
        xlswrite(outputfile,Mconfusion_avg_ensemble_tst,worksheet,'G44:I46');
        xlswrite(outputfile,Mconfusion_regress_ensemble_tst,worksheet,'G49:I51');
        xlswrite(outputfile,Mconfusion_step_ensemble_tst,worksheet,'G54:I56');
        xlswrite(outputfile,Mconfusion_net_ensemble_tst,worksheet,'G59:I61');
        xlswrite(outputfile,Mconfusion_vote_ensemble_tst,worksheet,'G64:I66');
        xlswrite(outputfile,Mconfusion_rbf_ensemble_tst,worksheet,'G69:I71');
        xlswrite(outputfile,Mconfusion_net2_ensemble_tst,worksheet,'G74:I76');
        xlswrite(outputfile,Mconfusion_net1_ensemble_tst,worksheet,'G79:I81');
    else
        SF3 = {'O1' 'O2' 'O3' 'O4'};
        SF4 = {'A1'; 'A2'; 'A3'; 'A4'};
        ...
        xlswrite(outputfile,{'Step-Tr'; 'A1'; 'A2'; 'A3'; 'A4'; '';...
            ...
            'A1'; 'A2'; 'A3';'A4';''; 'FFCNetEns-Tr';'A1'; 'A2'; 'A3';'A4'},worksheet,'A23:A93');
        ...
        xlswrite(outputfile,Mconfusion_avg_ensemble_tr,worksheet,'B48:E51');
        xlswrite(outputfile,Mconfusion_regress_ensemble_tr,worksheet,'B54:E57');
        xlswrite(outputfile,Mconfusion_step_ensemble_tr,worksheet,'B60:E63');
        xlswrite(outputfile,Mconfusion_net_ensemble_tr,worksheet,'B66:E69');
        xlswrite(outputfile,Mconfusion_vote_ensemble_tr,worksheet,'B72:E75');
        xlswrite(outputfile,Mconfusion_rbf_ensemble_tr,worksheet,'B78:E81');
        xlswrite(outputfile,Mconfusion_net2_ensemble_tr,worksheet,'B84:E87');
        xlswrite(outputfile,Mconfusion_net1_ensemble_tr,worksheet,'B90:E93');
        ...
        xlswrite(outputfile,{'Step-Tst'; 'A1'; 'A2'; 'A3'; 'A4'; '';...
            ...
            'A1'; 'A2'; 'A3';'A4';''; 'FFCNetEns-Tst';'A1'; 'A2'; 'A3';'A4'},worksheet,'G23:G93');
        ...
        xlswrite(outputfile,Mconfusion_avg_ensemble_tst,worksheet,'H48:K51');
        xlswrite(outputfile,Mconfusion_regress_ensemble_tst,worksheet,'H54:K57');
        xlswrite(outputfile,Mconfusion_step_ensemble_tst,worksheet,'H60:K63');
        xlswrite(outputfile,Mconfusion_net_ensemble_tst,worksheet,'H66:K69');
        xlswrite(outputfile,Mconfusion_vote_ensemble_tst,worksheet,'H72:K75');
        xlswrite(outputfile,Mconfusion_rbf_ensemble_tst,worksheet,'H78:K81');
        xlswrite(outputfile,Mconfusion_net2_ensemble_tst,worksheet,'H84:K87');
        xlswrite(outputfile,Mconfusion_net1_ensemble_tst,worksheet,'H90:K93');
    end;
    
    xlswrite(outputfile,SF,worksheet,'B1:G1');

    xlswrite(outputfile,SF2,worksheet,'A2:A13');
