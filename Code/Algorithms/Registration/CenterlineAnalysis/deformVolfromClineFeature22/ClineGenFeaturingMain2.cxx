...
int main( int argc, char * argv[] )
{
	...
	std::cout <<"     ************   markCnt = "<<markCnt<<"   ************"<<std::endl;
	srcMarkCnt = markCnt;
	...
	validCntZ = FindFeatures5(src, trg, m, window_size, plusSearchRange, minusSearchRange, mIndexPairOrg, fixed_correl);  // use Y-Z extrema respectively 	
	PrunedCnt = pruningIndex(mIndexPairOrg,  mIndexPairZ, validCntZ, MORE_window_size);
	...
	VecCline.fillFeaturedCline(PrunedCnt, MORE_window_size, TrgClineSize, src, trg, matchedPair,  mIndexPairZ);	
...
//	ObjVolume.fillGroupObjVoxel(trgMarkCnt, tVoxel, tX, tY, tZ);  // only fill the target object with DField from Cline DField
	ObjVolume.distFromClosestCline(VecCline);            				cout<< "after ObjVolume.distFromClosestCline(VecCline)"<<endl;
	ObjVolume.DFieldFromClosestCline(VecCline);                            cout<< "after ObjVolume.DFieldFromClosestCline(VecCline)"<<endl;
