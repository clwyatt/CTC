...
int GroupObjVoxel::CountSrcAndTrgFloatObj(TNT::Array3D<float> &Src, float srcThreshold, TNT::Array3D<float> &Trg, float trgThreshold, int dimx, int dimy, int dimz)
{
	int vol_num =0;
	...
	for(int i=0; i < dimx; i++){
		...
		if((Src[i][j][k] > srcThreshold) || (Trg[i][j][k] > trgThreshold) ) vol_num++;
	}
	}
	}
...
void GroupObjVoxel::fillGroupObjVoxelFloatObj(int num, TNT::Array3D<float> &Src, float srcThreshold, TNT::Array3D<float> &Trg, float trgThreshold, int dimx, int dimy, int dimz)
{
	...
	groupObjVoxel = new ObjVoxel[num];
		...
		for(int i=0; i < dimx; i++){
			...
			if( (Src[i][j][k] > srcThreshold) || (Trg[i][j][k] > trgThreshold) ) {
...
void GroupObjVoxel::fillGroupObjVoxelFloat(unsigned long int num, TNT::Array3D<float> &bVoxel, int dimx, int dimy, int dimz)
{
	unsigned long int vol =0;
	size = num;
	groupObjVoxel = new ObjVoxel[num];
...
void DField4Clines::storeClineDField(string &ClineVectorFile, ReaderType::Pointer reader, IndexPair* mIndexPair, int ClineSize)
{
	...
	typedef itk::Vector< float, Dimension >  VectorType;
	  ...
	  clog <<" Here? "<<endl;
	  TNT::Array3D<float> VecX(size[0], size[1], size[2], (float)0);           	  clog << "  TNT::Array3D<float> VecX "<<endl;
	  TNT::Array3D<float> VecY(size[0], size[1], size[2], (float)0);           clog << "  TNT::Array3D<float> VecY "<<endl;
	  TNT::Array3D<float> VecZ(size[0], size[1], size[2], (float)0);          clog << "  TNT::Array3D<float> VecZ "<<endl;
/*	  
...
*/	  
	  ...
	  clog << " ====   featurePair_size= "<< featurePair_size<< endl;
...
#endif	
	clog << "End of DField4Clines::storeClineDField"<<endl;  
}
