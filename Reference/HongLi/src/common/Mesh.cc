...
Mesh::MeshVertexSet::~MeshVertexSet()
{
	...
	if(shared){
		...
		std::slist<Vertex *>::iterator sli = linearList.begin();
...
Vertex * Mesh::MeshVertexSet::insert(Vertex v)
{
	...
	case true:
		...
		if(found){
			inlist = false;
			e = n->getData();
			std::slist<Vertex *>::iterator sli = e->verts.begin();
			while(sli != e->verts.end()){
				if(**sli == v){
					inlist = true;
					break;
...
bool Mesh::lineIntersection(Vertex &theVertex, Vertex &theIntersection, Vertex &closestMeshVertex)
{
	...
	t = sqrt((verticies->bounds[0])*(verticies->bounds[0]) + 
...
void Mesh::finalize()
{
	// cinvert vertex slist to vector
	// convert triangle slist to vector
	finalTlist.resize(triangles.size());

	std::slist<Triangle>::iterator trit = triangles.begin();
	size_t idx = 0;
...
void Mesh::ooglwrite(const char * name)
{
	...
	i = 0;
	while( mvi !=  vertexEnd() ){
		sprintf(entry, "%f %f %f %f %f %f\n", (*mvi)->getPX(), (*mvi)->getPY(), (*mvi)->getPZ(), (*mvi)->getNX(), (*mvi)->getNY(), (*mvi)->getNZ());
		outfile << entry;
		// label vertex 
		(*mvi)->label(i);
		i++;
