...
private:
	...
	slist<unsigned int> triNeighbors;
...
public:
	...
	/**
	* Get the distance from another Vertex 
	*
	* @return the distance
	*/
	 float distance(const Vertex & V){
		return (sqrt((px - V.px)*(px - V.px)
		 + (py - V.py)*(py - V.py)
		 + (pz - V.pz)*(pz - V.pz)));
