...
Create a training file from a csv database

-d file.csv specifies the file
-m (optional) mrn to process
-o features.csv specifies the output file
...
def process(db, mrn, study, series):
    ...
    # build directory
    basedir = os.getenv("CTC_IMG_DIR");
    basedir += "/"  + mrn + "/" + "%06i.STU/" % int(study) + "%06i.SER/" % int(series)

    # create polyp file
    pfile = open(basedir+"polyps", "w")
    for entry in db:
        if entry == mrn:
            for l in db[entry]:
                if (l[0] == study) & (l[1] == series) & (l[6] == "T"):
                    pfile.write("%s,%s,%s,%s\n" % (l[7],l[2], l[3], l[4]))
    pfile.close();
 ...
 #       return False
 #   command = os.getenv("CTC_DIR") + "/bin/fetool "
 #   command += "%s %s" % (indir, outbase)
...
def main():
    ...
    for entry in reader:
        if entry[0] == "MRN":
            continue
        if entry[0] in db:
            db[entry[0]].append(entry[1:9])
        else:
            db[entry[0]] = [entry[1:9]]
